...
[beta, approx_method, tol, kmax, nSamples, verbose, test_convergence, compute_marglik] = myProcessOptions(hyp, 'step_size', 0.1, 'approx_method', 'gauss_hermite', 'tol', 1e-4, 'max_iters', 100, 'nSamples', -1, 'verbose', 1, 'test_convergence', 1, 'compute_marglik', 1);
...
for k = 1:kmax
  ...
  tm1= r.*tm1+ (1-r).*( df-2*(dv.*post_m) );
  tm2 = r.*tm2 + (1-r).*(-2*dv);

  sW = sqrt(abs(tm2)) .* sign(tm2);
  L = chol(eye(n)+sW*sW'.*K); %L = chol(sW*K*sW + eye(n)); 
  V = L'\(repmat(sW,1,n).*K); %V  = L'\(sW*K);
  post_v = diag(K) - sum(V.*V,1)'; % v =diag(inv(inv(K)+diag(W))); %v^{k+1}
  pseudo_y = K*tm1+m;
  alpha_part=(sW.*(L\(L'\(sW.*pseudo_y))));
  post_m = pseudo_y - K*alpha_part;
  
  if compute_marglik
    % recompute alpha to reduce the noise E(lik) term 
    %alpha = (K+1e-10*eye(n))\(post_m-m);
    %alpha = (diag(abs(tm2))*K+eye(n))\(tm1-abs(tm2).*m);
    alpha = tm1 - alpha_part;
    ...
    tnu = tm1; ttau = tm2; 
    nlZ_ep = epComputeParams2(L,V,alpha,post_m,K,y,ttau,tnu,lik,hyp,m,'infEP');
...
if nargout>1                                             %do we want nlZ?
  % we return the EP approximation for now
  tnu = tm1; ttau = tm2; 
  nlZ = epComputeParams2(L,V,alpha,post_m,K,y,ttau,tnu,lik,hyp,m,'infEP');
...
function [nlZ] = epComputeParams2(L,V,alpha,mu,K,y,ttau,tnu,lik,hyp,m,inf)
  Sigma = K - V'*V;
  v = diag(Sigma);
